            ...
            <tbody>
                {tableData.map((task) =>{
                    <tr className='border-t border-gray-200' key={task._id}>
                        <td className='border-t border-gray-700 text-[13px] line-clamp-1 overflow-hidden'>{task.title}</td>
                        <td className='px-4 py-4'>
                            <span className={`px-2 py-1 rounded inline-block ${getStatusBadgeColor(task.status)}`}>{task.status}</span>
                        </td>
                        <td className='px-4 py-4'>
                            <span className={`px-2 py-1 rounded inline-block ${getStatusBadgeColor(task.priority)}`}>{task.priority}</span>
                        </td>
